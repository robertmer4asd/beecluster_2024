...
bool error, firstWeightingFlag = true;
...
static boolean rtcSync = false, rapFlag = false, firstWeightFlag = true, fl_rtcSyncSuccess = false;
...
void makeRtcSync(void){
  if(!rtcSync){
    gsm.RTCtime(&dayN,&monthN,&yearN,&hourN,&minuteN,&secondN);
    if(hourN > 0 && hourN < 23){
      rtc.adjust(DateTime(yearN, monthN, dayN, hourN , minuteN, secondN));
      fl_rtcSyncSuccess = true;
    }else if(hourN == 0 && minuteN == 0){
      Serial2.print("GSM CLOCK PROBLEM!"); 
      fl_rtcSyncSuccess = false;
    }
    rtcSync = true;
  }
}
