...
const char* topicFreq = "/Cantar/Robert/Frecventa";
const int port = 1883;
    ...
    case 5:
    {
      ...
      if (t - lastReconnectAttempt > 10000L) {
        ...
        if (mqttConnect()) {
          ...
          delay(1000);
          Serial2.println("Sound processing...");
          megaPowerOn();
          delay(17000);
          megaPowerOff();
          ...
          String lastString = ""; // Variable to store the last string read
          File dataFile = SD.open("data.txt");
          if (dataFile) {
            while (dataFile.available()) {
              lastString = dataFile.readStringUntil('\n'); // Read line from file
            }
            dataFile.close();
            
            Serial2.println("Last string from file: " + lastString);
            mqtt.publish(topicFreq, lastString.c_str(), 1);
            state = 1;
          }
...
void megaPowerOff(void)
{
  digitalWrite(MEGA_POWER, HIGH);
}

void megaPowerOn(void)
{
  digitalWrite(MEGA_POWER, LOW);
}
