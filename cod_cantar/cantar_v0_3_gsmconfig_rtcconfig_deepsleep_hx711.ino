    ...
    /*case 4:
    {
      ...
      if (modem.isNetworkConnected()) {
        ...
        if(mqtt.connected())
        {
          ...
          mqtt.publish(topicSignal, (char*)(String(signalRssi)).c_str(),1);
          if(rapFlag){
            mqtt.publish("ok", (char*)(String(masa)).c_str(),1);
            scale.tare();
          }
        }
        else{
...
boolean mqttConnect() {
  ...
  boolean status = mqtt.connect("Cantar_Robert", "mobilBog", "Ares");
  if (status == false) {
    Serial2.println(" fail");
    return false;
  }
  ...
  mqtt.publish(topicHello, "Hello Robert");
