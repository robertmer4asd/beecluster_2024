...
class DatabaseViewer(QMainWindow):
    def __init__(self, title):
        ...
        self.buttons = [
            ("Load Weight Database", self.load_weight_database),
            ("Load Temperature Database", self.load_temperature_database),
            ("Load Humidity Database", self.load_humidity_database),
            ("Load Frequency Database", self.load_freq_database),
            ("Show Databases", self.show_databases),
            ("Generate Graph", self.generate_graph),
            ("Generate Frequency Graph", self.generate_freq_graph),
            ("Predict Possible Events (DNN)", self.dnn_graph),
            ("Predict Possible Events (CNN)", self.cnn_graph),
            ("Exit", self.close)
        ]
        ...
        for btn_text, btn_func in self.buttons:
            ...
            button.setStyleSheet(
                "QPushButton { background-color: #4CAF50; color: white; border: none; padding: 10px 24px; font-size: 16px; border-radius: 8px; }"
